...
function buildPreviewData(selectedCase: any) {
  ...
  return {
    ...
    recommendation: student.personalized_feedback || selectedCase?.meta?.teacherNote || 'Teacher review required',
...
export function FeedbackTemplateManagement() {
  ...
  const reportData = useMemo(() => buildPreviewData(selectedCase), [selectedCase]);
  ...
  const handleDownloadHtml = () => {
    ...
    const html = [
      '<html><head><title>WriteLens report template</title></head><body>',
      `<h1>${settings.titlePrefix}</h1>`,
      `<p>${settings.openingNote}</p>`,
      `<pre>${JSON.stringify(reportData, null, 2)}</pre>`,
      `<p>${settings.closingNote}</p>`,
      '</body></html>',
    ].join('');
    ...
    link.download = 'writelens-report-template.html';
    link.click();
        ...
        <GlassCard className="p-6 border-[var(--border)] space-y-4">
          ...
          {selectedCase ? (
            <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-deep)] p-4 space-y-4">
              ...
              <ReportGenerator
                studentName={selectedCase.meta.studentName}
                studentId={selectedCase.meta.userId}
                data={reportData}
                onPrint={handlePrint}
